...
void iex2DObj::Render_reverseX( s32 DstX, s32 DstY, s32 DstW, s32 DstH, s32 SrcX, s32 SrcY, s32 width, s32 height, u32 dwFlags, COLOR color, float z )
{
	TLVERTEX	v[4];
	v[1].sx = v[3].sx = (float)DstX;
	v[0].sx = v[2].sx = (float)(DstX-DstW)-0.5f;
	v[0].sy = v[1].sy = (float)DstY;
	v[2].sy = v[3].sy = (float)(DstY+DstH)-0.5f;

	//反転させるとずれるので、ずれた分だけもどす
	float move = (float)this->width;
	v[1].sx += move;
	v[3].sx += move;
	v[0].sx += move;
	v[2].sx += move;
	...
	v[3].tu = v[1].tu = (float)(SrcX-0.5f)/(float)this->width;
...
void iex2DObj::rot_render(float rad)
{
	float srad = sin(rad), crad = cos(rad);
	float sentx, senty;
	float newx, newy;
...
void iex2DObj::rot_render(float rad, iexShader* shader, char* tech)
{
	float srad = sin(rad), crad = cos(rad);
	float sentx, senty;
	float newx, newy;
...
void iex2DObj::rot_render(float rad, s32 DstX, s32 DstY, s32 DstW, s32 DstH, s32 SrcX, s32 SrcY, s32 width, s32 height, u32 dwFlags, COLOR color, float z)
{
	float srad = sin(rad), crad = cos(rad);
	float sentx, senty;
	float newx, newy;
...
void iex2DObj::rot_render(float rad, s32 DstX, s32 DstY, s32 DstW, s32 DstH, s32 SrcX, s32 SrcY, s32 width, s32 height, iexShader* shader, char* tech, COLOR color, float z)
{
	float srad = sin(rad), crad = cos(rad);
	float sentx, senty;
	float newx, newy;
