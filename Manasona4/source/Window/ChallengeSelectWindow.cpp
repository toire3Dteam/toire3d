#include "ChallengeSelectWindow.h"
...
ChallengeSelectWindow::ChallengeSelectWindow(Vector2 vPos) :BaseWindow(vPos)
{
	m_pWindow = new tdn2DObj("Data/UI/Window/ChallengeSelectWindow.png");
	...
	AddIconData("チャレンジセレクト", "チャレンジ内容の変更を行います。");
...
ChallengeSelectWindow::~ChallengeSelectWindow()
{
...
bool ChallengeSelectWindow::Update()
{
	// 説明の下に乗せる板
	//m_pInfoPlate->Update();
...
void ChallengeSelectWindow::Redner()
{
	// 説明の下に乗せる板
	//m_pInfoPlate->Render();
	...
	for (int i = 0; i < (int)m_aIconData.size(); i++)
	{
		...
		// 枠
		//+------------------------------------------------	
		if (i == ChallengeSelectWindow::CHALLENGE_SELECT_STATE::SELECT_CHALLENGE)
		{
...
// 操作
bool  ChallengeSelectWindow::Ctrl(int DeviceID)
{
	// 起動していなかったらハジく
	if (m_bActive == false)return false;
	...
	// 決定
	//if ((CHALLENGE_SELECT_STATE)m_iSelectNo == CHALLENGE_SELECT_STATE::BACK)
	//{
	//	// ゲームに戻る動作は離して進む
	//	if (tdnInput::KeyGet(KEYCODE::KEY_B, DeviceID) == 2)
	//	{
	//		// 選択した番号格納！
	//		m_iChoiceState = (CHALLENGE_SELECT_STATE)m_iSelectNo;
	//		return true;
	//	}
	//}
	//else
	...
	//{
		...
		{
			// 選択した番号格納！
			m_iChoiceState = (CHALLENGE_SELECT_STATE::BACK);
			return true;
		}
	//}
...
void ChallengeSelectWindow::Action()
{
...
void ChallengeSelectWindow::Stop()
{
	BaseWindow::Stop();
...
void ChallengeSelectWindow::RenderParamSetting(int No, int x, int y)
{
	bool bSelect = false;
	if (No == m_iSelectNo)bSelect = true;


	switch ((CHALLENGE_SELECT_STATE)No)
	{
	case CHALLENGE_SELECT_STATE::SELECT_CHALLENGE:
		m_ChallengeSelectParam->Render(x, y, bSelect);
		break;
	case CHALLENGE_SELECT_STATE::BACK:
...
void ChallengeSelectWindow::CtrlParamSetting(int SelectNo, int DeviceID)
{
	switch ((CHALLENGE_SELECT_STATE)SelectNo)
	{
	case CHALLENGE_SELECT_STATE::SELECT_CHALLENGE:
