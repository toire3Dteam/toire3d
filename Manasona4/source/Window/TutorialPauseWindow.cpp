...
// 操作
bool  TutorialPauseWindow::Ctrl(int DeviceID)
{
	// 起動していなかったらハジく
	if (m_bActive == false)return false;
	...
	// 決定
	if ((TUTORIAL_PAUSE_STATE)m_iSelectNo == TUTORIAL_PAUSE_STATE::BACK)
	{
		// ゲームに戻る動作は離して進む
		if (tdnInput::KeyGet(KEYCODE::KEY_B, DeviceID) == 2)
		{
			// 選択した番号格納！
			m_iChoiceState = (TUTORIAL_PAUSE_STATE)m_iSelectNo;
			return true;
		}
	}
	else
