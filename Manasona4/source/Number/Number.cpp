...
void Number::Render(int x, int y, int num, NUM_KIND kind,RS lpRs)
{
	...
	case Number::NUM_KIND::NORMAL:
		...
		for (int count = 0;; ++count)
		{
			...
			m_pic->Render(x - ((count * (m_picSize /2))*m_picScale), y, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
	...
	case Number::NUM_KIND::HEAL:
		for (int count = 0;; ++count)
		{
			...
			m_pic->Render(x - ((count *32)*m_picScale), y, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize,lpRs);// 数字描画
	...
	case Number::NUM_KIND::TIMER:
	{
		...
		for (;; ++count)
		{
			...
			m_pic->Render(x - ((count * (int)(32* PiccScale))*m_picScale), y, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画

			if (number <= 0)break;// 数値が０以下になったらさよなら
		}

		m_pic->Render(x - ((++count * (34 * (int)PiccScale))*m_picScale)+8, y, m_picSize*m_picScale, m_picSize*m_picScale, 10 * m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
		m_pic->Render(x - ((++count * (36 * (int)PiccScale))*m_picScale)+8, y, m_picSize*m_picScale, m_picSize*m_picScale, 11 * m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
	...
	case Number::NUM_KIND::COMBO:
		for (int count = 0;; ++count)
		{
			...
			//m_pic->SetAngle(0.3f);
			m_pic->Render(x - ((count * (m_picSize / 2))*m_picScale), y /*+ count*20*/, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
	...
	case Number::NUM_KIND::DAMAGE_SCORE:
		...
		for (int count = 0;; ++count)
		{
			...
			m_pic->Render(x - ((count * (int)(m_picSize / 1.5f))*m_picScale), y, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
	...
	case Number::NUM_KIND::PARSENT:
	{
		...
		for (;; ++count)
		{
			...
			m_pic->Render(x - ((count * (int)((float)m_picSize / 1.75f))*m_picScale), y, m_picSize*m_picScale, m_picSize*m_picScale, digitNum*m_picSize, 0, m_picSize, m_picSize, lpRs);// 数字描画
	...
	case Number::NUM_KIND::SECOND:
	{
		...
		m_pic->Render(x + ((((int)((float)m_picSize / 1.75f)))*m_picScale) + 4, y, m_picSize*m_picScale, m_picSize*m_picScale, (14 * m_picSize), 0, m_picSize, m_picSize, lpRs);// 秒
