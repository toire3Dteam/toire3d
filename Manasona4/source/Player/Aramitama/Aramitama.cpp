...
void Aramitama::InitActionDatas()
{
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::RUSH1].pAttackData->damage = 220;
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::RUSH1].pAttackData->places[(int)AttackData::HIT_PLACE::LAND].HitRecoveryFrame = 24;
	m_ActionDatas[(int)BASE_ACTION_STATE::RUSH1].pAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].HitRecoveryFrame = 24;
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::RUSH2].pAttackData->damage = 280;
	...
	// 判定形状
	m_ActionDatas[(int)BASE_ACTION_STATE::ANTI_AIR].pAttackData->pCollisionShape->width = 9.5f;
	m_ActionDatas[(int)BASE_ACTION_STATE::ANTI_AIR].pAttackData->pCollisionShape->height = 12;
	m_ActionDatas[(int)BASE_ACTION_STATE::ANTI_AIR].pAttackData->pCollisionShape->pos.Set(4.5f, 14, 0);

	//==============================================================================================================
	//	中断攻撃(どっこいアタック)
	// 地上ヒットも空中ヒットも共通の情報
	m_ActionDatas[(int)BASE_ACTION_STATE::DOKKOI_ATTACK].InstanceAttackData();	// アタックデータ作成
	m_ActionDatas[(int)BASE_ACTION_STATE::DOKKOI_ATTACK].pAttackData->damage = 600;
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::DOKKOI_ATTACK].pAttackData->pCollisionShape->pos.Set(5, 5.5f, 0);
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::INVINCIBLE_ATTACK].pAttackData->damage = 340;
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::INVINCIBLE_ATTACK].pAttackData->places[(int)AttackData::HIT_PLACE::LAND].FlyVector.Set(.95f, 2.5f);
	m_ActionDatas[(int)BASE_ACTION_STATE::INVINCIBLE_ATTACK].pAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].FlyVector.Set(1.25f, 2.5f);
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::INVINCIBLE_ATTACK].pAttackData->pCollisionShape->pos.Set(4, 8, 0);
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::SKILL_AERIAL].pAttackData->fRepeatAttackRate = 0.95f;
	// 地上ヒットと空中ヒットで挙動が変わるもの
	...
	// 判定形状								   
	m_ActionDatas[(int)BASE_ACTION_STATE::SKILL_AERIAL].pAttackData->pCollisionShape->width = 11;
	m_ActionDatas[(int)BASE_ACTION_STATE::SKILL_AERIAL].pAttackData->pCollisionShape->height = 11;
	m_ActionDatas[(int)BASE_ACTION_STATE::SKILL_AERIAL].pAttackData->pCollisionShape->pos.Set(2, 7, 0);
	...
	// 判定形状
	m_ActionDatas[(int)BASE_ACTION_STATE::THROW].pAttackData->pCollisionShape->width = 5.5;
	...
	m_ActionDatas[(int)BASE_ACTION_STATE::THROW_SUCCESS].pAttackData->places[(int)AttackData::HIT_PLACE::LAND].FlyVector.Set(1.0f, 2.2f);
	m_ActionDatas[(int)BASE_ACTION_STATE::THROW_SUCCESS].pAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].FlyVector.Set(1.0f, 2.3f);
...
void Aramitama::InitMushiDatas()
{
	...
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->ShakeCameraInfo.Set(.4f, 4);
	...
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->places[(int)AttackData::HIT_PLACE::LAND].FlyVector.Set(.75f, 2.5f);
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].FlyVector.Set(.75f, 2.5f);
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->places[(int)AttackData::HIT_PLACE::LAND].iHitStopFrame = 25;
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].iHitStopFrame = 25;
	...
	// 判定形状
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->pCollisionShape->width = 14;
	m_pMushi[(int)MUSHI_TYPE::LAND]->tagpAttackData->pCollisionShape->height = 18;
	...
	m_pMushi[(int)MUSHI_TYPE::SQUAT]->tagpAttackData->HitSE = "ヒット1";
	...
	m_pMushi[(int)MUSHI_TYPE::AERIAL]->tagpAttackData->places[(int)AttackData::HIT_PLACE::LAND].FlyVector.Set(.01f, 1.0f);
	...
	m_pMushi[(int)MUSHI_TYPE::AERIAL]->tagpAttackData->places[(int)AttackData::HIT_PLACE::LAND].HitRecoveryFrame = 45;
	m_pMushi[(int)MUSHI_TYPE::AERIAL]->tagpAttackData->places[(int)AttackData::HIT_PLACE::AERIAL].HitRecoveryFrame = 45;
	...
	// 判定形状
	m_pMushi[(int)MUSHI_TYPE::AERIAL]->tagpAttackData->pCollisionShape->width = 10;
	m_pMushi[(int)MUSHI_TYPE::AERIAL]->tagpAttackData->pCollisionShape->height = 6;
...
bool Aramitama::SkillAction::Squat::Execute()
{
	...
	{
		// ゲージ増加
		m_pAramitama->AddWassyoiGauge(m_pAramitama->c_WASSYOIGAUGE_MAX / 5);	// 20%
...
void Aramitama::SkillAction::Aerial::Enter()
{
	...
	if (m_pAramitama->GetMove().y < 0) m_pAramitama->AddMove(Vector3(0, -m_pAramitama->GetMove().y * 4, 0));
...
bool Aramitama::SkillAction::Aerial::Execute()
{
	...
	if (!m_bMoved)
	{
		m_pAramitama->SetMove(m_pAramitama->GetMove() * .75f);

		// 攻撃判定出たら
		if (m_pAramitama->isActiveFrame())
		{
			//m_pAramitama->SetMoveUpdate(true);
			m_pAramitama->SetMove(Vector3(m_pAramitama->GetDirVecX() * 1.1f, 2.0f, 0));

			// 動いたフラグを立てて、もう入らないようにする
			m_bMoved = true;

			// めり込みフラグON
			m_pAramitama->SetSinkingUpdate(false);
		}
	}
